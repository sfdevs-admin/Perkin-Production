...
export default class B2bCartUploadModal extends LightningModal {
    ...
    fileData;
    errorMessage = 'Unfortunately some line items could not be processed due to failures.  Please reupload only the corrected failed lines and not the complete file.';
    ...
    onGetCartSummary(result) {
        if (result.data && result.data.uniqueProductCount > 0) {
            ...
            if(this.allowedProductToAdd == 0){
                this.showToast('Your Cart is full','Max 500 items can be added to your cart.','sticky','error');
                return ;
            }
        }
    }
    ...
    downloadSampleCSV(){
        ...
        this.downloadCSV(csv,'Sample Bulk Upload');
    }
    ...
    async processCartItems(cartItemsMap){
        try{
            ...
            if(Object.keys(cartItemInputRequest).length > this.allowedProductToAdd){
                this.showToast('Your Cart is full, A maximum of 500 line items can be added to cart.','Currently '+this.allowedProductToAdd+' items can be added to your cart, please checkout current cart and try again. ','sticky','error');
            ...
            this.isLoading = false;
            console.error(ex);
            this.showToast('Some Error Occured while adding items to cart',ex,'sticky','error');
        }
    }
