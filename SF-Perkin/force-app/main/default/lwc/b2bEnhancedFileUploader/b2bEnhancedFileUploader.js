...
import getKey from '@salesforce/apex/B2B_EnhancedFileUploaderController.getKey';
import encrypt from '@salesforce/apex/B2B_EnhancedFileUploaderController.encrypt';
...
export default class B2bEnhancedFileUploader extends NavigationMixin(LightningElement) {
    ...
    fetchExistingFiles() {
        console.log('inside fetchExistingFiles--- ', this.recordId);
    ...
    handleUploadFinished(files) {
        ...
        if(this.recordId && versIds.length > 0){
            createContentDocLink({versIds: versIds, encodedKey: this.currentCartId, visibleToAllUsers: this.visibleToAllUsers})
                .then((res) => {
    ...
    communicateEvent(docIds, versIds, fileNames, objFiles){
        ...
        this.dispatchEvent(new CustomEvent('fileupload', {bubbles: true, composed: true,
 detail: fileNames
 }));
        this.loading = false;
    ...
    openFile(event) {
        ...
        //     attributes: {
        //         pageName: 'contentversion'
        //         // pageName: 'filePreview'contentversion
        //     },
        //     state: {
        //         recordIds: docId
        //     }
    ...
    deleteDocumentUponError(versIds){
        ...
        deleteAllContentDoc({versIds: versIds})
        .then(() => {
