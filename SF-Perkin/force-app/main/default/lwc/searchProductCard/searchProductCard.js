...
import { CartStatusAdapter, CartSummaryAdapter } from 'commerce/cartApi';
...
export default class SearchProductCard extends LightningElement {
    ...
    /**
     * Gets  or sets the product data from ProductCollectionAdapter.
     */
    @api
    ...
    // get pricingInfo() {
    //     const prices = this.displayData?.prices || {};
    //     const b2bPromoList = this.displayData?.b2bPromotionalPrices || [];

    //     // find the promo data for this product (match productId with displayData.id)
    //     const promo = b2bPromoList.find(p => p.productId === this.displayData.id) || {};

    //     const hasPromo =
    //         Array.isArray(promo.promotionPriceAdjustmentList) &&
    //         promo.promotionPriceAdjustmentList.length > 0;

    //     return {
    //         negotiatedPrice: hasPromo
    //             ? parseFloat(promo.promotionalPrice ?? 0).toFixed(2)
    //             : parseFloat(prices.negotiatedPrice ?? 0).toFixed(2),

    //         listingPrice: hasPromo ? parseFloat(promo.salesPrice ?? 0).toFixed(2) : parseFloat(prices.listingPrice ?? 0).toFixed(2),

    //         savedAmount: hasPromo
    //             ? Math.abs(parseFloat(promo.promotionPriceAdjustmentList[0].adjustmentAmount)).toFixed(2)
    //             : null,
    //         promotionSalesPrice: hasPromo
    //             ? parseFloat(promo.salesPrice ?? 0).toFixed(2)
    //             : null,

    //         currencyIsoCode: promo.currencyIsoCode || prices.currencyIsoCode || '',
    //         isLoading: !!prices.isLoading
    //     };
    // }

    // updated code when negotiated prices are not available 
    get pricingInfo() {
        const prices = this.displayData?.prices || {};
        const b2bPromoList = this.displayData?.b2bPromotionalPrices || [];

        // find the promo data for this product
        const promo = b2bPromoList.find(p => p.productId === this.displayData.id) || {};

        // only consider promo if negotiatedPrice exists
        const negotiatedPriceExists = prices.negotiatedPrice != null || prices.negotiatedPrice !== undefined;
        ...
        return {
            negotiatedPrice: parseFloat(
                hasPromo
                    ? promo.promotionalPrice ?? prices.negotiatedPrice ?? 0
                    : prices.negotiatedPrice ?? 0
            ).toFixed(2),

            // listingPrice: hasPromo
            //     ? parseFloat(promo.salesPrice ?? 0).toFixed(2)
            //     : parseFloat(prices.listingPrice ?? 0).toFixed(2),
            listingPrice: parseFloat(
                hasPromo && promo.salesPrice != null
                    ? promo.salesPrice
                    : prices.listingPrice ?? 0
            ).toFixed(2),
     ...
     */
    get fields() {
        return (this.displayData?.fields ?? [])
        .filter(field => field.name !== 'B2B_Product_Line__c')
        .map((field) => {
            let temp = field;
            return {
                displayData: field,
    ...
    get showCallToActionButton() {
        return ( this.configuration?.showCallToActionButton && !! ( parseInt( this.displayData?.prices?.negotiatedPrice ) ) ) ?? false;
    }
    ...
    get unavailablePriceLabel() {
        return i18n.unavailablePriceLabel;
    }
    renderedCallback() {
        calculateImageSizes(this.querySelector('.imageArea'), this._imageSizes);
    }
