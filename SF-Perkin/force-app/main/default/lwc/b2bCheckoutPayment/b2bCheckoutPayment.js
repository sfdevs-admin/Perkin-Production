...
export default class B2bCheckoutPayment extends NavigationMixin(useCheckoutComponent(LightningElement)) {
    ...
    paymentOptions = [
        ...
        { label: 'Submit Purchase Order', value: 'po' },
    ];
                ...
                if (!this.stripePaymentDone) {
                    if (this.paymentOption == 'paynow') {
                        if (this.customerId != null) {
                            ...
                            this.errorMessage = 'User must have an Email to Checkout.';
                            this.showCheckoutError(this.errorMessage);
                        }
                    }
    ...
    checkValidity() {
        ...
        if (this.showReqError) {
            this.errorMessage = 'Please enter a valid purchase order number.';
            this.showCheckoutError(this.errorMessage);
            return false;
        }
    ...
    async processPaymentFunction() {
        ...
        try {
            if (this.isValidBillingAddress) {
                ...
                this.showSpinner = false;
                this.errorMessage = 'Billing address missing, please contact System Administrator.';
                this.showCheckoutError(this.errorMessage);
                return false;
            }
        } catch (error) {
